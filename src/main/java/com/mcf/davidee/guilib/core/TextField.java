...
public abstract class TextField extends FocusableWidget {
	...
	protected void drawCursorVertical(int x1, int y1, int x2, int y2) {
		...
		tessellator.getWorldRenderer().pos(x1, y2, 0.0D);
		tessellator.getWorldRenderer().pos((double) x2, (double) y2, 0.0D);
		tessellator.getWorldRenderer().pos((double) x2, (double) y1, 0.0D);
		tessellator.getWorldRenderer().pos((double) x1, (double) y1, 0.0D).endVertex();
		tessellator.draw();
